...
export function parseCreateCmdOutput(out: string): string | null {
  ...
  let regex = /BagID = (?<bagId>.{64})/
  let res = out.match(regex)
...
export class TonStorageService {
  async createBag(filePath1: string): Promise<{ bagId: string } | string> {
    ...
    const ls = spawn(
      `${process.env.STORAGE_CLI_EXEC_PATH}`, [
        ...
        '-c', `\"create -d CreatedFromNest '${filePath}'\"`,
      ], {
    ...
    return new Promise((resolve, reject) => {
      ...
      ls.on('close', (code) => {
        ...
        let bagId = parseCreateCmdOutput(consoleOut.toString())
        if (bagId) {
          resolve({bagId})
        } else {
  ...
  async createContract(bagId: string, providerAddress: string): Promise<NodeJS.ArrayBufferView> {
    let tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "create-contract"))
    let file = path.join(tempDir, 'provider-response')
    ...
    const ls = spawn(
      `${process.env.STORAGE_CLI_EXEC_PATH}`, [
        ...
        '-k', 'storage-db/cli-keys/client',
        '-p', 'storage-db/cli-keys/server.pub',
        '-c', `\"new-contract-message ${bagId} ${file.replace(/\\/g, "/")} --provider ${providerAddress}\"`,
      ], {
        cwd: `${process.env.STORAGE_WORK_DIR}`,
        shell: process.env.USE_SHELL === "true",
        windowsVerbatimArguments: true
