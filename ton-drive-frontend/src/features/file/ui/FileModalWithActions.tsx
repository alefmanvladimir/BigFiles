...
export default forwardRef(function FileModalWithActions(
  ...
  const fileDownloadStatus = useFileDownload(file?.bagId!!)
  const downloadElement = fileDownloadStatus.ready ? <a
      download href={`http://localhost:3000/download/${file?.bagId}/${fileDownloadStatus.fileName}`} target="_blank"
    ...
    </a> : (!fileDownloadStatus.downloadRequested ? <button
    onClick={fileDownloadStatus.prepareLink}
    className="join-item btn btn-outline btn-success">
    Prepare Download
  </button> : <span className="loading loading-bars loading-sm"/>)
  function closeItem (bagId: string) {
        ...
        <>
          {
            file &&
              <div className="flex flex-wrap gap-2">
                <button className="join-item btn btn-outline btn-error" onClick={() => closeItem(file.bagId)}>
                  {/* trash icon */}
